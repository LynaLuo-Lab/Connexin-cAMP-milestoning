...
load('E:\LuoLAB\cx26\cAMP\27cAMP_strip\clustering_number\neg200mv_cAMP_COM.mat')
...
for i = 1 : 9919
    ...
    for j = 0 : 26
        if j < 10
            eval(['space(j+1, :) = neg200mv_cAMP_COM{1}.Vec_0000' num2str(j) '(i, :);']);
        else
            eval(['space(j+1, :) = neg200mv_cAMP_COM{1}.Vec_000' num2str(j) '(i, :);']);
        end
    end
    space(:, 3) = space(:, 3) - 74; % offset by protein COM z =74A
    
    index = find(space(:, 3) >= 30 & space(:, 3) < 40);
